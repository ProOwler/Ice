...
class CSSmin
{
    ...
    public function run($css = '', $linebreak_pos = false)
    {
        ...
        while (($start_index = $this->index_of($css, '/*', $start_index)) >= 0) {
            ...
            $css = $this->str_slice($css, 0, $start_index + 2) . $comment_preserve_string . $this->str_slice(
                    $css,
                    $end_index
                );
    ...
    private function minify($css, $linebreak_pos)
    {
        ...
        for ($i = 0, $max = count($this->comments); $i < $max; $i++) {
            ...
            if (substr($token, (strlen($token) - 1), 1) === '\\') {
                ...
                $css = preg_replace(
                    '/' . self::COMMENT . $i . '___/',
                    self::TOKEN . (count($this->preserved_tokens) - 1) . '___',
                    $css,
                    1
                );
                continue;
        ...
        $css = preg_replace_callback(
            ...
            array($this, 'lowercase_directives'),
            $css
        );
        ...
        $css = preg_replace_callback(
            ...
            array($this, 'lowercase_pseudo_elements'),
            $css
        );
        ...
        $css = preg_replace_callback(
            ...
            array($this, 'lowercase_common_functions'),
            $css
        );
        ...
        $css = preg_replace_callback(
            ...
            array($this, 'lowercase_common_functions_values'),
            $css
        );
        ...
        $css = preg_replace(
            '/(^|[^0-9])(?:0?\.)?0(?:em|ex|ch|rem|vw|vh|vm|vmin|cm|mm|in|px|pt|pc|%|deg|g?rad|m?s|k?hz)/iS',
            '${1}0',
            $css
        );
        ...
        $css = preg_replace(
            '/(background\-position|webkit-mask-position|(?:webkit|moz|o|ms|)\-?transform\-origin)\:0(;|\}| \!)/iS',
            '$1:0 0$2',
            $css
        );
